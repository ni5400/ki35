...
Class Image {
	 ...
	 * @return [boolean]
	 */
	Static Public function code ($length = NULL, $width = NULL, $height = NULL) {
		...
		$len = strlen($seed) - 1;
		for ($i = 0; $i < 20; $i++) {
			$color = imagecolorallocate($verify, mt_rand(100, 255), mt_rand(100, 255), mt_rand(100, 255));
		...
		$_SESSION[$name] = md5(strtolower($code));
		...
		for ($i = 0, $h = $height / 2 - 2; $i < 5; $i++, $h++) {
			...
			imageline($verify, 0, 0, $width, $height, $color);
		}
